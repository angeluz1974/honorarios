...
Define Class ctlStructure As Exception
	...
	Protected Procedure ReadCharArray(m.pnAddress, m.pnLength)
		*!* Reads a string array of m.pnLength, stops at first nul char
		...
		Return Left(m.lcString , At(Chr(0), m.lcString) - 1)
	Endproc
	...
	Protected Procedure ReadPCString(m.pnAddress)
		...
		*!* Read value of string pointer, then reads string
		If m.lnLength > 0 Then
			...
			m.lcString = Left(m.lcString , At(Chr(0), m.lcString) - 1)
		Else
			m.lcString = ""
		Endif
		Return m.lcString
	...
	Protected Procedure WriteMemPointer(m.pnAddress, m.pnInt)
		*!* Writes an integer (4 bytes)
...
Define Class ctlDrawItemStruct As ctlStructure
	...
	Procedure hDC_Assign(m.lnNewVal)
		This.WriteMemPointer(This.Address + 24, m.lnNewVal)
	Endproc
...
*!*	*LPURL_COMPONENTS;
********************************************************************************
Define Class ctlUrlComponents As ctlStructure
...
*!*	  *LPWIN32_FIND_DATA;
********************************************************************************
Define Class ctlWin32FindData As ctlStructure
	SizeOf = 320
	...
	Procedure cFileName_Assign(m.lnNewVal)
		This.WriteCharArray(This.Address + 44, Padr(m.lnNewVal, MAX_PATH, Chr(0)), MAX_PATH)
	Endproc
	...
	Procedure cAlternateFileName_Assign(m.lnNewVal)
		This.WriteCharArray(This.Address + 44 + MAX_PATH, Padr(m.lnNewVal, 14, Chr(0)), 14)
	Endproc
...
Define Class ctlLogFont As ctlStructure
	...
	Procedure FontName_Assign(m.vNewVal)
		If Right(m.vNewVal, 1) <> Chr(0) Then
			m.vNewVal = m.vNewVal + Chr(0)
		Endif
